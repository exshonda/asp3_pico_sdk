...
/*
 *  タイマドライバのターゲット依存部（RaspberryPi Pico 2 ASP3用）
 */
 ...
 *  タイマ割込みハンドラ登録のための定数
 */
#define INTNO_TIMER  (RP2350_TIMER0_0_IRQn + 16) /* 割込み番号 */
#define INHNO_TIMER  (RP2350_TIMER0_0_IRQn + 16) /* 割込みハンドラ番号 */
...
Inline void target_hrt_set_event(HRTCNT hrtcnt)
{
    ...
    if (target_hrt_get_current() - current >= hrtcnt) {
        ...
        sil_wrw_mem((uint32_t *)NVIC_ISPR0, INTNO_2_IRQBIT(INTNO_TIMER));
    }
}
