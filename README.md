...
> 設計（割込み・タイマ共存）・実測データ・経緯の記録は [docs/](docs/README.md) を参照。
...
> リポジトリ内（`asp3/asp3_pico_sdk.cmake`・`asp3/target/...`）で吸収済みで、利用者の操作は上記の切り替えのみです。
...
```cmake
# 配置に応じてパスを調整（本リポジトリでは sample1 から見て ../asp3/）
